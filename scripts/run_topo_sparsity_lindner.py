"""Lindner-aware topological sparsity analysis (dimension-aware defaults).

Runs all 4 experiments from topo_sparsity_analysis.ipynb with the new
dimension-aware threshold from commit 3c16832 (max(10*d, 100) for d=10 → 100).
Also captures TopologyDimensionalityWarning in Experiment 1.
Saves figures to figures/lindner/ and prints all metrics for capture.
"""
...
FIGURES_DIR = os.path.join(REPO_ROOT, "figures", "lindner")
os.makedirs(FIGURES_DIR, exist_ok=True)
DATA_PATH = os.path.join(REPO_ROOT, "data", "transformer", "math500_hidden_states.npz")
RESULTS_PATH = os.path.join(REPO_ROOT, "data", "transformer", "lindner_results.json")

# ========== LOAD DATA ==========
print("=" * 60)
print("LINDNER-AWARE RUN (dimension-aware threshold)")
print("=" * 60)
...
# ========== EXPERIMENT 1: Point Cloud PH + TopologyDimensionalityWarning ==========
print("\n--- Experiment 1: Point Cloud Topology (with dimensionality warnings) ---")

N_PCA_COMPONENTS = 50
exp1_results = {}
exp1_analyzers = {}
exp1_pca_info = {}
exp1_dim_warnings = {}  # level -> bool (whether TopologyDimensionalityWarning fired)
exp1_eff_ranks = {}     # level -> effective_rank from PH result
...
for level in range(1, 6):
    ...
    # Capture TopologyDimensionalityWarning
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        result = pa.fit_transform(cloud_pca, subsample=min(n_k, 200), seed=42)
        dim_warned = any(issubclass(x.category, TopologyDimensionalityWarning) for x in w)
        exp1_dim_warnings[level] = dim_warned
        exp1_eff_ranks[level] = int(result.get("effective_rank", -1))
        if dim_warned:
            print(f"  ** TopologyDimensionalityWarning fired (eff_rank={result.get('effective_rank', '?')})")
...
print("\nTopologyDimensionalityWarning summary:")
for lv in range(1, 6):
    print(f"  Level {lv}: warned={exp1_dim_warnings[lv]}, eff_rank={exp1_eff_ranks[lv]}")
...
for i, level in enumerate(range(1, 6)):
    ...
    warned_str = " *" if exp1_dim_warnings[level] else ""
    ax.set_title(f"Level {level}{warned_str}")
    ax.set_xlabel("Birth")
    if i == 0:
        ax.set_ylabel("Death")
    ax.legend(fontsize=8)
    ax.set_aspect("equal")
fig.suptitle("Persistence Diagrams by Difficulty Level (LINDNER)", fontsize=14, y=1.02)
fig.tight_layout()
...
ax.set_title("H1 Persistence Entropy vs Task Difficulty (LINDNER)")
...
ax.set_title("PCA Variance Concentration vs Difficulty (LINDNER)")
ax.set_xticks(range(1, 6))
...
ax.set_title("PI(Level 5) - PI(Level 1), H1 (LINDNER)")
...
# ========== EXPERIMENT 2: Trajectory Topology (LINDNER: dimension-aware) ==========
print("\n--- Experiment 2: Token Trajectory Topology (LINDNER dimension-aware) ---")
...
for level in range(1, 6):
    ...
    for idx in level_indices:
        ...
        # LINDNER: use dimension-aware default (no explicit threshold)
    ...
    print(f"Level {level}: {n_total} traj, {degen_rate:.0%} degen, mean cond={mean_cond:.1f}, mean PE(H1)={mean_pe:.4f}, threshold={val['threshold_used']:.0f}")

# Exp 2 summary table
print("\nLevel | Degen Rate | Mean Cond | Mean PE(H1) | Mean #H1 Features | Threshold")
print("------|------------|-----------|-------------|-------------------|----------")
for lv in range(1, 6):
    ...
    if results:
        ...
        thr = results[0]["threshold_used"]
        print(f"  {lv}   | {dr:>9.0%} | {mc:>9.1f} | {mp:>11.4f} | {mf:>17.1f} | {thr:>8.0f}")
...
ax.set_title("Trajectory H1 Entropy vs Difficulty (LINDNER)"); ax.set_xticks(level_labels); ax.grid(True, alpha=0.3)
...
ax.set_title("Trajectory Condition Number vs Difficulty (LINDNER)"); ax.set_xticks(level_labels); ax.grid(True, alpha=0.3)
...
ax.set_title("Trajectory Degeneracy Rate vs Difficulty (LINDNER)"); ax.set_xticks(level_labels)
...
ax.set_title("Layer-wise Topological Transition (LINDNER)"); ax.legend(); ax.grid(True, alpha=0.3)
...
ax.set_title("Layer-wise H1 Complexity (LINDNER)"); ax.legend(); ax.grid(True, alpha=0.3)
...
ax.set_title("Pairwise Wasserstein Distance (LINDNER)")
fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
for i in range(5):
    for j in range(5):
...
ax.set_title(f"Permutation Null (n={N_PERMUTATIONS}), p={p_value:.4f} (LINDNER)"); ax.legend()
...
for level in range(1, 6):
    ...
    summary[level] = {
        ...
        "dim_warned": exp1_dim_warnings[level],
        "exp1_eff_rank": exp1_eff_ranks[level],
    }

cols = ["L1 Norm", "Top10% E", "Eff Rank", "PE(H1)", "PCA@50", "Cond #", "Degen %", "DimWarn"]
keys = ["l1_norm", "top10_energy", "eff_rank", "pers_entropy_h1", "pca_var_50", "cond_number", "degen_rate", "dim_warned"]
...
for level in range(1, 6):
    ...
    for k in keys:
        ...
        if k == "degen_rate":
            ...
            vals.append(f"{'YES':>9s}" if v else f"{'no':>9s}")
        else:
...
metric_keys_corr = ["l1_norm", "top10_energy", "eff_rank", "pers_entropy_h1", "pca_var_50", "cond_number"]
metric_arrays = {}
for k in metric_keys_corr:
...
print()
for i, (name_i, key_i) in enumerate(zip(metric_names, metric_keys_corr)):
    print(f"{name_i:>16s}", end="")
    for j, (name_j, key_j) in enumerate(zip(metric_names, metric_keys_corr)):
...
results_out = {
    "run_type": "lindner",
    "condition_threshold": "dimension-aware (max(10*d, 100))",
    ...
    "exp1_dim_warnings": {str(lv): exp1_dim_warnings[lv] for lv in range(1, 6)},
    "exp1_eff_ranks": {str(lv): exp1_eff_ranks[lv] for lv in range(1, 6)},
