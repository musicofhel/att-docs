...
The transition detection pipeline is validated on both synthetic bistable EEG (Experiment~6) and real binocular rivalry EEG from a single subject (Experiment~7).
While the $N = 1$ real-data validation demonstrates feasibility and high precision, multi-subject analysis is required to establish generalizability across individuals, electrode montages, and rivalry paradigms.
...
\begin{itemize}
    \item \textbf{Multi-subject neural validation}: The $N = 1$ real EEG validation (Experiment~7) demonstrates feasibility on the \citet{nie2023accumulating} dataset. The full dataset comprises 84 subjects with multiple rivalry epochs each, enabling systematic analysis of precision--recall tradeoffs across subjects, electrode sites, and CUSUM threshold settings. Cross-region binding detection during stable percepts vs.\ transition periods is a natural extension.
