...
To validate the transition detector on real neural data, we apply it to the binocular rivalry SSVEP dataset of \citet{nie2023accumulating}, which provides concurrent EEG and behavioral reports of perceptual alternations during binocular rivalry.

We analyze Subject~1, Epoch~0, using the Oz (occipital midline) channel bandpass-filtered to the theta--alpha range (4--13\,Hz) to isolate rivalry-relevant oscillatory dynamics.
The epoch contains 43{,}196 samples at 360\,Hz (120\,s), with 41 behavioral perceptual switches reported by the subject (29 clear alternations between left and right percepts, 12 involving a mixed/piecemeal percept).
...
We analyze Subject~1, Epoch~0 from the \citet{nie2023accumulating} dataset, using channels Oz (occipital midline) and Pz (parietal midline), bandpass-filtered to 4--13\,Hz.
The \texttt{BindingDetector} is configured with per-channel delay estimation (delay $\tau = 12$, embedding dimension $m = 5$), and applied in a sliding-window fashion: 10\,s windows with 5\,s step, yielding 23 overlapping windows across the 120\,s epoch.
...
The score is non-zero at $\epsilon = 0$ ($S = 184.2$), reflecting inherent mismatch between joint and marginal persistence image representations even for independent systems.
...
\textbf{Preliminary real-data validation}: The binding detection experiments (1--5) use coupled chaotic systems with known ground truth.
The transition detection pipeline is validated on both synthetic bistable EEG (Experiment~6) and real binocular rivalry EEG from a single subject (Experiment~7), and cross-region binding is validated on the same subject's Oz--Pz channels (Experiment~8).
While the $N = 1$ real-data validation demonstrates feasibility and significant correlations with behavior, multi-subject analysis is required to establish generalizability across individuals, electrode montages, and rivalry paradigms.
...
\begin{itemize}
    \item \textbf{Multi-subject neural validation}: The $N = 1$ real EEG validation (Experiments~7--8) demonstrates feasibility on the \citet{nie2023accumulating} dataset. The full dataset comprises 84 subjects with multiple rivalry epochs each, enabling systematic analysis of precision--recall tradeoffs across subjects, electrode sites, and CUSUM threshold settings. Extending the cross-region binding analysis (Experiment~8) to all electrode pairs would yield a full topological connectivity matrix, complementing functional connectivity measures based on coherence or Granger causality.
    \item \textbf{R-Cross-Barcodes}: The R-Cross-Barcode construction provides a theoretically cleaner way to identify features present in one filtration but absent from another. Extending this from graphs to Vietoris--Rips complexes on point clouds would provide a stronger theoretical foundation.
    \item \textbf{Diagram matching}: Hungarian algorithm matching between joint and marginal persistence diagrams as an alternative to image subtraction.
    \item \textbf{Multi-system binding}: Extension to $N > 2$ coupled systems, detecting higher-order interactions.
\end{itemize}


\section{Software Availability}

The \texttt{att-toolkit} Python package is available at \url{https://github.com/musicofhel/att-toolkit}.
