...
and (viii) z-score calibration against surrogate null distributions, revealing that the method is selectively sensitive to heterogeneous-timescale coupling (R\"ossler--Lorenz: 3/5 seeds significant at $p \leq 0.05$) while having zero power for same-timescale coupling (Lorenz--Lorenz: all z-scores negative).
...
\textbf{Preliminary real-data validation}: The EEG results (Experiments~7--8) are from a single subject ($N = 1$), a single epoch, and two electrodes.
Multi-subject analysis is required to establish generalizability.
...
\begin{itemize}
    \item \textbf{Multi-subject neural validation}: The full \citet{nie2023accumulating} dataset comprises 84 subjects, enabling group-level precision--recall analysis and full topological connectivity matrices across all electrode pairs.
