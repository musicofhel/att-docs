...
\textbf{Real-data validation scope}: The multi-subject transition detection (Experiment~7, $N = 80$) provides population-level evidence for high-precision changepoint detection. However, the cross-region binding analysis (Experiment~8) remains single-subject ($N = 1$), single-epoch, and two-electrode.
...
\begin{itemize}
    \item \textbf{Multi-subject cross-region binding}: While multi-subject transition detection is now complete ($N = 80$, Experiment~7), extending the cross-region binding analysis (Experiment~8) across all subjects and electrode pairs would enable full topological connectivity matrices.
